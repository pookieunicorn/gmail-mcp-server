...
class GmailService:
    ...
    async def get_unread_emails(self, max_results: int = 1) -> list[dict[str, str]] | str:
        """
        Retrieves up to `max_results` unread messages from mailbox.
...
async def main(creds_file_path: str,
    ...
    async def handle_list_tools() -> list[types.Tool]:
        return [
            ...
            types.Tool(
                name="open-email",
                description="Display the content of the email in the chat",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "email_id": {
    ...
    async def handle_call_tool(
        ...
        if name == "get-unread-emails":
            ...
            unread_emails = await gmail_service.get_unread_emails(max_results=1)
            if unread_emails:
                email_id = unread_emails[0]['id']
                email_data = await gmail_service.read_email(email_id)
                return [types.TextContent(type="text", text=str(email_data),artifact={"type": "dictionary", "data": email_data} )]
        ...
        if name == "open-email":
            ...
            # Fetch and display the email content instead of opening a browser
            email_data = await gmail_service.read_email(email_id)
            return [types.TextContent(type="text", text=str(email_data), artifact={"type": "dictionary", "data": email_data})]
        if name == "trash-email":
            email_id = arguments.get("email_id")
        ...
        if name == "search-emails":
            ...
            response = await asyncio.to_thread(
                gmail_service.service.users().messages().list(
                    userId="me", q=query, maxResults=max_results
                ).execute
            )
            messages = response.get("messages", [])
            return [types.TextContent(type="text", text=str(messages), artifact={"type": "list", "data": messages})]
        else:
